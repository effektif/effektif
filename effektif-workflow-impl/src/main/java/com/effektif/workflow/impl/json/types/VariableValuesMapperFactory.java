 ...
 * @author Tom Baeyens
 */
public class VariableValuesMapperFactory implements JsonTypeMapperFactory {
  ...
  public JsonTypeMapper createTypeMapper(Type type, Class< ? > clazz, Mappings mappings) {
    if (clazz==VariableValues.class) {
      ...
      TypedValueMapper typedValueMapper = new TypedValueMapper(typedMapping);
      return new VariableValuesMapper(new MapMapper(typedValueMapper));
    }
    return null;
  }
}
